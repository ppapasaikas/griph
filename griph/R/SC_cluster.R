...
SC_cluster <- function(DM, use.par=FALSE,ncores="all",is.cor = FALSE,
    ...
    if (!is.null(ncom)){
        ...
        else{                   #Otherwise run simple kmeans on a dimD projection generated by largeVis
        message(memb$algorithm, " returned too few clusters...Switching to k-nn...")
        x=projectKNNs(as_adj(GRAO,names=FALSE, sparse=TRUE), dim=6, sgd_batches=min(20000/(sum(ADJ!=0)/2),0.99), M=3, gamma=10, alpha=0.1, useDegree=TRUE)  
