...
SC_cluster <- function(DM, use.par=FALSE,ncores="all",is.cor = FALSE,
    ...
    min.csize <-max(4, ceiling(0.2*sqrt(length(memb)) ) )
        if (!is.null(comm$memberships) & max(  apply(comm$memberships,1,function(x) sum(table(x) > min.csize ))  ) >= ncom ){    #check if multi level louvain was used and a leve has enough substantial clusters
        nclust=apply(comm$memberships,1,function(x) sum(table(x) > min.csize ))
