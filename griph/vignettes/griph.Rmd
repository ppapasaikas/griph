...
# Visualizing \code{griph} results
\code{SC_cluster} returns an \code{igraph} with the cells represented by graph verteces
(\code{GRAO} and \code{plotGRAO}, which is a simplified, plotting optimized version of \code{GRAO}).
This allows to easily plot the graph again, for example to separately illustrate known and
predicted cell types:  
```{r knownVsPred, message=FALSE, fig.show='hide'}
...
```{r buettner_groupPolygons, out.height="600px", out.width="600px", message=FALSE, fig.show='hide'}
...
```{r buettner_collapseSameClass, out.height="600px", out.width="600px", message=FALSE, fig.show='hide'}
