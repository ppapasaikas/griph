...
abstract: >
    ...
    from griph is SC_cluster. The vignette illustrates its use
...
Cell types can be identified using *SC_cluster*:  
...
`r CRANpkg("igraph")` objects in the return value of *SC_cluster*:  
```{r graphreturned}
res$GRAO # full graph
res$plotGRAO # simplified graph
```


# Visualizing `r Githubpkg("ppapasaikas/griph")` results
In the graphs returned by *SC_cluster*, cells are represented by graph verteces.
...
the *plotGRAO* component in the return value of *SC_cluster* is `NULL` and
...
or more conveniently using the wrapper function provided by `grip`:  
```{r buettner_tsne2}
plotTsne(res, fill.type = "predicted", line.type = "none", mark.type = "true")
```

# Accessing the cell classification
*SC_cluster* identified `r length(unique(res$MEMB))`
