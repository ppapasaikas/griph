...
library(griph)
res <- griph_cluster(M, ClassAssignment = trueLabel, plotG = TRUE, fsuffix='tung')
```
```{r tung_types}
table(res$MEMB)
```
...
summary(res$plotGRAO$GRAO) # simplified graph
```
...
t-SNE to the affinity matrix returned by `r Githubpkg("ppapasaikas/griph")`:  
```{r tung_tsne}
library(Rtsne)
...
matrix above where for example *NA190998* cells are found in multiple predicted classes. `r Githubpkg("ppapasaikas/griph")`
