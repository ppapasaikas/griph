...
switched off here. \code{\link{SC_cluster}} identified `r length(unique(res$MEMB))`
...
plot.GRAO contais an optimized for rendering version of the complete graph. Edges are pruned, vertex attributes are added in order to visualize predicted and know class assignment (if given) and vertices are sampled if they exceed the maxG argument.

Alternatively (and preferably for large - >1000 - numbers of cells ) results can be visualized by applying a dimensionality reduction/projection technique such as tSNE to the affinity
