...
SC_cluster <- function(DM,is.cor=FALSE,impute=TRUE,filter=TRUE,diffuse.iter=3,rho=0.2,batch.penalty=0.5,ClassAssignment=rep(1,ncol(DM)),BatchAssignment=NULL,plotG=TRUE,plotSP=TRUE,fsuffix=RandString() ){
...
if (corMethod=="pearson") {C[[1]]=pcor(log2(DM+1))}
else {C[[1]]=Spcor(log2(DM+1))}

p=max(2,ceiling (0.5/ IQR((C[[1]])[upper.tri(C[[1]])])/mean(C[[1]][upper.tri(C[[1]])])) )
p=min(8,p)
C[[1]]=gausskernel( (C[[1]]^p),sigma=1)  #Kernelized distance metric 
...
A=gausskernel(ADJ^0.2,sigma=1+ 1/(bw.nrd0 ( as.vector( (ADJ)[upper.tri(ADJ)]) ) +1e-2)  )
